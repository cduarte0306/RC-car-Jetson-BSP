---
name: Sub-Issue Template
about: Create a sub-issue for an existing parent issue
title: '[SUB] '
labels: 'sub-issue'
assignees: ''
---

**Parent Issue:** #

## Description
<!-- Provide a clear description of this sub-task -->

## Acceptance Criteria
<!-- Define what needs to be done for this sub-issue to be considered complete -->
- [ ] Add specific acceptance criteria here 

## Additional Context
<!-- Add any other context, screenshots, or information about the sub-issue here -->

## Related Files/Components
<!-- List any files, components, or areas of the codebase this sub-issue affects -->
